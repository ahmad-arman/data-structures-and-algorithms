...
const howMuchPencil = (str) => {
  let result = [];
  // Solution code here...
  let arrForCh = str.split('');
  result.push(str);
  arrForCh.forEach((element,index) => {

    let cutting = result[index].slice(1);
    result.push(cutting);
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  let ing = recipe.ingredients.slice();
  ing.forEach((element) =>{
    let first = element.slice();
    let sec = first.split(' ');
    let three = sec.slice(2, 6);
    let four = three.join(' ');
    result.push(four);
  });
  return result;
