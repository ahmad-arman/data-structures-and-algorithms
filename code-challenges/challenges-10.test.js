...
const count = (target, input) => {
  // Solution code here...
  let number =0;
  let regex = new RegExp(`\\b${target}\\b`, 'i');

  // console.log(regex);
  let newArr = input.join(',').split(',');

  newArr.forEach (element => {
    let checkNumber = regex.test(element);
    if (checkNumber)
      number ++;
  });
  // console.log(number);
...
const totalSum = (input) => {
  // Solution code here...
  let array= input.reduce( ( acc, val ) => acc.concat(val),
    []
  );
  let ahmad = array.reduce((acc,val) =>{
    return acc + val;
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(element =>{
    ...
    arrayOne = element.filter(value => value %5 === 0 && typeof value === 'number');

    arrayOne.forEach((element, index )=> arrayOne[index]= Math.pow(2,element));
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newObject=data.filter(starWarsData => {
    if(starWarsData.gender==='female' || starWarsData.gender==='male' )
    {
      return starWarsData.name;
    }
  });

  let newArray = newObject.map(element=>
  {
    return element.name;
  });
...
let findShortest = (data) => {
  // Solution code here...
  let newObject=data.filter(starWarsData => {
    if(starWarsData.gender!=='female' & starWarsData.gender!=='male' & starWarsData.name !=='C-3PO')
    {
      return starWarsData.name;
    }
  });
